...
export const CALCULATE_VEVE_LICENSORS_METRICS = async () => {
    try {
        ...
        for (const licensor of licensors){
            const collectibles = licensor.veve_collectibles;
            let marketCap = 0
            let totalListings = 0
            let allTimeHigh = 0;
            let allTimeLow = 0;
            ...
            if (licensor.licensor_id) {

                await prisma.veve_licensors_metrics.upsert({
                    where: {
                ...
                console.log(`[UPDATED] ${licensor.name} market cap updated to ${marketCap.toFixed(2)}`);

            } else {
                console.log('NO LICENSOR ID ', licensor)
            }
