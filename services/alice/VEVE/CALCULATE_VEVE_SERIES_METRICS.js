...
export const CALCULATE_VEVE_SERIES_METRICS = async () => {
    try {

        const seriesX = await prisma.veve_series.findMany({
            include: {
                veve_collectibles: {
                    include: {
                        metrics: {
        ...
        for (const series of seriesX) {
            const collectibles = series.veve_collectibles;
            let marketCap = 0
            let totalListings = 0
            let allTimeHigh = 0;
            let allTimeLow = 0;
            ...
            if (series.series_id){
                ...
                await prisma.veve_series_metrics.upsert({
                    where: {
                ...
                console.log(`[UPDATED] ${series.name} market cap updated to ${marketCap.toFixed(2)}`);

            } else {
                console.log('NO SERIES ID ', series)
            }
